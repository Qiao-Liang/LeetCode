...
class Solution:
    def possibleBipartition(self, N, dislikes):
        ...
        def dfs(node, group):
            ...
            groups[node] = group
            return all(dfs(neb, group ^ 1) for neb in graph[node])

        return all(dfs(node, 0) for node in range(1, N + 1) if node not in groups)
