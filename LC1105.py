class Solution(object):
    def minHeightShelves(self, books, shelf_width):
        ...
        # len_books = len(books)
        # w, h = books[0]
        # new_line = [None] * len_books
        # same_line = [None] * len_books
        # new_line[0] = same_line[0] = (h, shelf_width - w, h)

        # for i, (w, h) in enumerate(books[1:], start=1):
        #     sh, sw, smh = same_line[i - 1]
        #     nh, nw, nmh = new_line[i - 1]
        #     tsh = sh
        #     tnh = nh
        #     same_line[i] = new_line[i] = (min(sh, nh) + h, shelf_width - w, h)
        ...
        #         if tsh < same_line[i][0]:
        #             same_line[i] = (tsh, sw - w, smh)
        ...
        #         if tnh < same_line[i][0]:
        #             same_line[i] = (tnh, nw - w, nmh)

        # return min(same_line[-1][0], new_line[-1][0])
    ...
    def minHeightShelves2(self, books, shelf_width):
        ...
        def dfs(bi, rw, h):
            if bi >= len_books:
                ...
                lbh = books[bi][1]
                bw, bh = books[bi]
                temp = min(dfs(bi + 1, shelf_width - bw, h + bh), dfs(bi + 1, rw - bw, max(h, h - lbh + bh)) if rw >= bw else float('inf'))
                self.dp[bi] = temp
                return temp
